...
We want our turtles to hold a pen, so that they can draw the path as they travel. Rather than creating a brand new class, we'll subclass our `turtle` class in order to reuse what we created (make life easier). We specify the class as a character vector in a specific-to-general order: `turtle_with_pen` and then `turtle`. They share the same underlying data, but `turtle_with_pen` holds some metadata (or attributes) that describe how the path to be drawn in terms of colour and thickness.
...
turtle_with_pen <- function(x = turtle(), colour = "steelblue", thickness = 1,
  ...
  new_rcrd(vec_data(x),
    ...
    class = c("turtle_with_pen", "turtle"))
}
turtle_with_pen(x)
```
...
It looks like stationary turtles are being displayed again. The reason is that the default `on = FALSE` directs to calling `NextMethod()`, which essentially calls `print.turtle()`. We should have switched them `on` in the first place. Our pen-holding turtles inherit `forward()` and `turn()` behaviours from `turtle`, and conduct a hexagonal walk. 
