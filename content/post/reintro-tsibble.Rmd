...
The development of the [**tsibble**](http://github.com/tidyverts/tsibble) package has been taking place since July 2017, and its `v0.6.2` has recently landed on CRAN. Yup, it's already `v0.6.2` and has evolved rapidly. As I become more confident about *tsibble*'s data structure and software design, I have removed the lifecycle badge, thus declaring the matururity of this versions. I have [introduced tsibble](https://blog.earo.me/2018/02/06/tsibble-or-tibbletime/) before in comparison with another package. Now I'd like to reintroduce **tsibble** (bold for package) to you and highlight the role *tsibble* (italic for data structure) plays in tidy time series analysis.
...
Current time series objects are simplistic data representations tailored for models. We are expected to learn a different set of tools from what we are used to (saying the tidyverse) in order to transform and visualise time series. By contrast, the choice of *tsibble*'s abstraction is made from a data-centric perspective, which is taken in the light of the operations that are to be performed on the data. Users who are already familiar with the tidyverse, will experience a gentle learning curve for mastering [**tsibble**](https://pkg.earo.me/tsibble/reference/index.html) and glide into temporal data analysis with low cognitive load.
...
Some types of operation assume an input in a strict temporal order, such as `lag()`, `lead()` and `difference()`. Hence the ordering of data rows is arranged in the first place by the key and then index from past to recent.
...
* Modifying the key variable results in an invalid *tsibble*.
...
*Tsibble* knows the index coupled with time zone, and therefore a shorthand `filter_index()` here for conveniently filtering time. Initially I was reluctant to add this feature, but it is too verbose when involving time zone specification. The resulting equivalent saves us a few keystrokes without losing much expressiveness.
...
In spirit of `group_by()`, `index_by()` takes effect in conjunction with subsequent operations for example `summarise()`. This is how we achieve to compute the number of trips every hour. Have you noticed that the displayed interval switches from irregular `[!]` to one hour `[1h]`?
...
There are a handful of functions [handling implicit missing values](https://pkg.earo.me/tsibble/articles/implicit-na.html), all suffixed with `_gaps()`:
...
Hourly tallies for winter and summer months are laid out in the calendar format. The bike usage is primarily driven by commuters in working days. The pattern on July 4th pops out like a weekend due to Independence day in US. No surprise, New Yorkers bike more in summer than winter time.
...
A large chunk of **tsibble**'s functions dedicated to performing rolling window operations are left untouched in this post. You may like to read more about rolling window [here](https://pkg.earo.me/tsibble/articles/window.html). I have added parallel support to them in `v0.6.2`, prefixed with `future_` by taking advantage of the awesome **furrr** package.

The **tsibble** package forms the infrastructure for new forecasting software, [**fable**](https://github.com/tidyverts/fable). I will speak at rstudio::conf in January, with forecasting *tsibble* (goodbye to `ts`) in action.
