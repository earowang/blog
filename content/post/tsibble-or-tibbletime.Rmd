...
Temporal data often has missing timestamps. Tsibble provides a helper function `fill_na()` to make these missing timestamps explicit, but tibbletime doesn't currently have similar capabilities. 
...
The function `fill()` is re-exported from tidyr and used to replace explicit missing values with the previous or next observation. For example, the previous hour's temperature can be filled in for each weather station. 
...
The tsibble approach introduces a time series specific summarise function--`tsummarise()`. The prefix "t" indicates the first name-value pair, which defines how the time index is collapsed to higher-level periods and can also be named. The remaining "summarise" components work like `summarise()`. This function takes advantages of several [lubridate](http://lubridate.tidyverse.org/) functions, such as `year()` for yearly aggregation, `as_date()` for daily, and `ceiling_date()` for other options. How about quarterly? Check out `yearquarter()`. Zoo's `as.yearmth()` and `as.yearqtr()` also work here!
...
Some variants of `tsummarise()`, such as `tsummarise_all()`, `tsummarise_at()`, and `tsummarise_if()`, will be implemented in a near future release.
